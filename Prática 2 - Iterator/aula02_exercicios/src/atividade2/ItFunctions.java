...
public class ItFunctions {
	//Iteradores fornecem uma vida para acessar os elementos de um conjunto sequencialmente sem expor sua implementação em si.
	//Simplificam a interface do Agregado, permitindo que mais de um caminhamento seja realizado simultaneamente.
	//Sua criação e manutenção pode trazer um custo adicional em termos de memória e tempo.
	...
	static boolean isNonDecreasing(List<Integer> list) {
		...
		if (!it.hasNext()) {
            return true; // Lista vazia é considerada não decrescente
        }
