...
public abstract class MainGUIFramework extends JFrame {
	...
	private AbstractAction drawEditAction, drawRemoveAction, drawNewCircleAction, drawNewLineAction,
			drawNewRectangleAction, undoAction, redoAction, exitAction, fileNewAction, fileOpenAction, fileSaveAction,
			fileSaveAsAction;
	private JFileChooser fileChooser;
	private MyFileFilter filter;
	private JDesktopPane desktop;

	private class InternalFrame extends JInternalFrame implements Observer {
	...
	public MainGUIFramework() {
		synchronized (MainGUIFramework.class) {
			...
			if (!permiteConstrutor) {
				throw new RuntimeException("N�o permite acessar construtor!");
			}
	...
	public void tryClose() {
		...
		for (JInternalFrame f : frames) {
			...
			// se algum frame n�o for realmente fechado,
			// ent�o a aplica��o nao ser� encerrada
			if (!f.isClosed()) {
				return;
			}
		}
	...
	private void refresh() {
		...
		drawNewLineAction.setEnabled(appDoc != null);
	...
	private boolean verifModified(AppDoc appDoc) {
		...
		int resp = JOptionPane.showConfirmDialog(this,
				"Arquivo " + appDoc.getURL() + " foi modificado - deseja gravar?", "Verifica��o",
